...
## 玄関ボード（天気・ごみ）
時計の下に、気象庁の予報（東京地方の降水確率、東京の気温）と、多摩市 緑地区（貝取1〜5丁目）のごみ収集を表示します。
- 天気はアプリがブラウザから直接取得します。圏外のときは表示されません。
- ごみの規則は `index.html` 内の `GOMI` に曜日規則で書いてあります。**毎年 4 月に市のカレンダーと照らして `validUntil` と規則を更新**してください。
- 各カードの「位置を見る」は、バスナビ.com のその停留所のバス位置情報ページを開きます。
